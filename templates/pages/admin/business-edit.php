...
<section class="max-w-5xl mx-auto rounded-[2rem] border border-red-100 bg-white p-6 md:p-8 shadow-xl shadow-red-900/10 space-y-6">
    ...
    <form class="grid gap-4 md:grid-cols-2" action="/panel/negocio/editar" method="post" enctype="multipart/form-data">
        ...
        <label class="block"><span class="block text-sm text-gray-700 mb-2">Código postal</span><input name="postal_code" type="text" value="<?= htmlspecialchars((string) ($old['postal_code'] ?? ''), ENT_QUOTES, 'UTF-8') ?>" placeholder="Ej: 1702" class="w-full rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3 text-gray-900 outline-none transition-all focus:bg-white focus:border-red-600 focus:ring-4 focus:ring-red-600/20"><?php if (($formErrors['postal_code'] ?? null) !== null) : ?><span class="mt-2 block text-sm text-rose-600"><?= htmlspecialchars((string) $formErrors['postal_code'], ENT_QUOTES, 'UTF-8') ?></span><?php endif; ?></label>
        <label class="block md:col-span-2"><span class="block text-sm text-gray-700 mb-2">Entre calles <span class="text-gray-400 font-normal">(Opcional)</span></span><input name="between_streets" type="text" value="<?= htmlspecialchars((string) ($old['between_streets'] ?? ''), ENT_QUOTES, 'UTF-8') ?>" placeholder="Ej: Entre Av. Gaona y Rivadavia" class="w-full rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3 text-gray-900 outline-none transition-all focus:bg-white focus:border-red-600 focus:ring-4 focus:ring-red-600/20"></label>
