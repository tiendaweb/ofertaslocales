...
<section class="max-w-5xl mx-auto rounded-[2rem] border border-red-100 bg-white p-6 md:p-8 shadow-xl shadow-red-900/10 space-y-6">
    ...
    <form class="grid gap-4 md:grid-cols-2" action="/panel/negocio/editar" method="post" enctype="multipart/form-data">
        ...
        <label class="block md:col-span-2"><span class="block text-sm text-gray-700 mb-2">Entre calles</span><input name="between_streets" type="text" value="<?= htmlspecialchars((string) ($old['between_streets'] ?? ''), ENT_QUOTES, 'UTF-8') ?>" placeholder="Ej: Entre Av. Díaz Vélez y Rivadavia" class="w-full rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3 text-gray-900 outline-none transition-all focus:bg-white focus:border-red-600 focus:ring-4 focus:ring-red-600/20"></label>
